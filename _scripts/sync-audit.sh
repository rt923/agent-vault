#!/usr/bin/env bash
# sync-audit.sh — 從 meta_peg_agent/ 同步審計數據到 agent-vault
#
# 用法:
...
# 依賴:
#   - bash 4+
#   - python3（用於解析 JSONL 和計算聚合）
#   - 可選: jq（若無則用 python3 替代）
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --only-dashboards)
            ...
            ;;
        -h|--help)
            echo "用法: bash _scripts/sync-audit.sh -s /path/to/meta_peg_agent [選項]"
            echo "選項:"
            echo "  -s, --source         meta_peg_agent 目錄路徑（必填）"
            echo "  --only-gates         僅同步閘門事件"
            echo "  --only-proposals     僅同步提案"
            echo "  --only-fixes         僅同步修復報告"
            echo "  --only-dashboards    僅更新看板"
            exit 0
            ;;
        *)
            echo "未知參數: $1"
...
if [[ -z "$SOURCE_DIR" ]]; then
    echo -e "${RED}錯誤: 請指定 -s /path/to/meta_peg_agent${NC}"
    exit 1
fi
if [[ ! -d "$SOURCE_DIR" ]]; then
    echo -e "${RED}錯誤: 來源目錄不存在: $SOURCE_DIR${NC}"
    exit 1
fi
if [[ ! -d "$SOURCE_DIR/logs" ]]; then
    echo -e "${YELLOW}警告: 來源目錄沒有 logs/ 子目錄${NC}"
fi

echo -e "${BLUE}=== 同步審計數據 ===${NC}"
echo -e "來源: $SOURCE_DIR"
echo -e "目標: $VAULT_DIR"
echo -e "模式: $MODE"
echo ""
...
# ---- 1. 同步閘門事件 ----
sync_gates() {
    echo -e "${GREEN}[1/4] 同步閘門事件...${NC}"
    local logs_dir="$SOURCE_DIR/logs"
    local gate_dir="$VAULT_DIR/_audit/_gate-events"

    if [[ ! -d "$logs_dir" ]]; then
        echo -e "${YELLOW}  跳過：無 logs/ 目錄${NC}"
        return
    fi

    # 使用 python3 解析每條 JSONL 並生成 Obsidian 筆記
    python3 -c "
import os, json, sys, hashlib

logs_dir = os.path.abspath('$logs_dir')
gate_dir = os.path.abspath('$gate_dir')
...
for fname in sorted(os.listdir(logs_dir)):
    ...
    with open(fpath, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # 解析檔名
            # gate_20260714_125558_73_PASS_20bceea9a4fd1304.jsonl
...
# ---- 2. 同步提案 ----
sync_proposals() {
    echo -e "${GREEN}[2/4] 同步自指提案...${NC}"
    local drafts_dir="$SOURCE_DIR/drafts"
    local props_dir="$VAULT_DIR/_audit/_proposals"

    if [[ ! -d "$drafts_dir" ]]; then
        echo -e "${YELLOW}  跳過：無 drafts/ 目錄${NC}"
        return
    fi
...
with open(fpath, 'r', encoding='utf-8') as f:
    content = f.read()
# 添加 Obsidian 相容的 frontmatter 包裝（如果沒有）
if not content.startswith('---'):
    content = '---\ntitle: \"$fname\"\nsync_from: \"drafts/$fname\"\ntags:\n  - proposal\n  - sync\n---\n\n' + content
...
# ---- 3. 同步修復報告 ----
sync_fixes() {
    echo -e "${GREEN}[3/4] 同步修復報告...${NC}"
    local fixes_dir="$SOURCE_DIR/fix_reports"
    local target_dir="$VAULT_DIR/_audit/_fix-reports"

    if [[ ! -d "$fixes_dir" ]]; then
        echo -e "${YELLOW}  跳過：無 fix_reports/ 目錄${NC}"
        return
    fi
...
# ---- 4. 更新看板 ----
sync_dashboards() {
    echo -e "${GREEN}[4/4] 更新聚合看板...${NC}"
    local gate_dir="$VAULT_DIR/_audit/_gate-events"
    local dash_dir="$VAULT_DIR/_audit/_dashboards"
...
gate_dir = os.path.abspath('$gate_dir')
dash_dir = os.path.abspath('$dash_dir')
...
trend_lines.append('')
trend_lines.append('> [!info] 由 \`sync-audit.sh\` 自動更新')
...
posture_lines.append('')
posture_lines.append('> [!info] 由 \`sync-audit.sh\` 自動更新')
...
echo -e "在 Obsidian 中按 Ctrl+R 重新整理檔案列表即可查看更新。"